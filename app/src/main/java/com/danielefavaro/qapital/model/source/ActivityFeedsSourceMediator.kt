...
class ActivityFeedsSourceMediator @Inject constructor(
...
) : RemoteMediator<Int, ActivityFeedModel>() {

    private val to: Date = Calendar.getInstance().time

    override suspend fun load(loadType: LoadType, state: PagingState<Int, ActivityFeedModel>): MediatorResult {
        val loadTypeResult = when (loadType) {
            LoadType.REFRESH -> {
        ...
        return try {
            ...
            val (updatedPosition, activityFeedList) = getActivityListModel(loadTypeResult as Date)

            val (oldest, feeds) = Pair(
                activityFeedList.oldest,
                activityFeedList.activities.map { feedModel ->
                    val user: UserModel? = try {
                        getUser(feedModel.userId)
                    } catch (_: Exception) {
                        null
                    }
     ...
     */
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ActivityFeedModel>): ActivityFeedRemoteKeys? =
     ...
     */
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ActivityFeedModel>): ActivityFeedRemoteKeys? =
     ...
     */
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ActivityFeedModel>): ActivityFeedRemoteKeys? =
    ...
    private suspend fun getUser(userId: Long): UserModel = service.getUser(userId)
     ...
     */
    private suspend fun getActivityListModel(position: Date): Pair<Date, ActivityListModel> {
        val activityListModel = service.getActivityFeeds(from = position.plusTwoWeeks().toApiFormat(), to = position.toApiFormat())
        val endOfPagination: Boolean = position.time <= activityListModel.oldest.time
        return if (activityListModel.activities.isEmpty() && !endOfPagination) {
            getActivityListModel(position.plusTwoWeeks())
        } else Pair(position, activityListModel)
    }
